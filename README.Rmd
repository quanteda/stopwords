...
We now make it easy to edit your own stopword lists, using the interactive editor.  For instance to edit the English stopword list for the Snowball source:
...
This also works for stopwords whose underlying structure is a lists, such as the "marimo" source:
```{r eval = FALSE}
# edit the English stopwords
...
Note that as of version 2.0, we now re-export `%>%` operator from **magrittr** it possible to chaining operations without loading additional packages.

## Adding stopwords to your own package

As of version 2, we've made it a one-step process to add `stopwords()` to your package through a re-export.  Simply call `use_stopwords()` like this:
